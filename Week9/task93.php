...
if(isset($_REQUEST["maker"])){
    ...
    if($js==$cars[$i]["maker"]){
        echo "<div class = 'card'><img src='",$cars[$i]['image'],"'style='width:100px;height:100px;'/><div class = 'description'><div class = 'title'>",$cars[$i]['maker']," ",$cars[$i]['model'],"</div><div class = 'price'>",$cars[$i]['price'],"$</div><div class = 'year'>",2018-$cars[$i]['year']," years</div></div></div>";
    }
}
}
else if(isset($_REQUEST["year"])){
	for($i=0;$i<sizeof($cars);$i++){
    if(2018-7>$cars[$i]["year"]){
        echo "<div class = 'card'><img src='",$cars[$i]['image'],"'style='width:100px;height:100px;'/><div class = 'description'><div class = 'title'>",$cars[$i]['maker']," ",$cars[$i]['model'],"</div><div class = 'price'>",$cars[$i]['price'],"$</div><div class = 'year'>",2018-$cars[$i]['year']," years</div></div></div>";
    }
}}
else if(isset($_REQUEST["price"])){
	for($i=0;$i<sizeof($cars);$i++){
    if(20000<$cars[$i]["price"]){
        echo "<div class = 'card'><img src='",$cars[$i]['image'],"'style='width:100px;height:100px;'/><div class = 'description'><div class = 'title'>",$cars[$i]['maker']," ",$cars[$i]['model'],"</div><div class = 'price'>",$cars[$i]['price'],"$</div><div class = 'year'>",2018-$cars[$i]['year']," years</div></div></div>";
    }
}
}
